...
#### 在线监控RN页面的性能指标FCP
开发者可以通过以下方式在原生侧获取RN页面首帧监控能力，用于性能优化。下列是两种实现方式，分别是将`mount`事件、`onLayout`事件的触发时间作为监控RN页面的首帧时间，由开发者自由选择。
...
以下是一些可以提升运行性能的编译选项，您可以按需在应用顶层 CMakeLists.txt 中添加
...
> NOTE: --aarch64-inline-plt 选项是毕昇新增选项，使用 clang 构建时添加此选项会报错

选项说明：

- **`-flto`** (编译，链接)：启用链接时优化，在链接阶段进行全程序分析和优化，可以跨编译单元进行内联、常量传播等优化
- **`-fwhole-program-vtables`**：如果所有虚函数调用都可见，允许编译器将虚函数调用去虚化为直接函数调用，消除虚函数调用开销并启用进一步的内联优化
- **`-wholeprogramdevirt-check=fallback`**：配置去虚化失败时的回退策略，确保优化失败时程序仍能正确运行
- **`--enable-partial-inlining`** (编译，链接)：允许编译器只内联函数的热路径部分，避免完全内联导致的代码膨胀
- **`--aarch64-inline-plt`**：在ARM64架构上内联过程链接表，将间接跳转优化为直接跳转
- **`--tail-dup-placement`**：复制代码块尾部到多个位置，减少跳转指令并改善指令缓存局部性
...
使用PGO优化鸿蒙应用需要使用 5.1.0.828 或更高版本的 DevEco Studio，共四步操作

1. 插桩编译：这一步骤的目的是构建有收集应用运行信息能力的hap。需要在应用入口模块的 src/main/cpp/CMakeLists.txt 中临时添加下述构建参数。注意添加的参数只对添加位置后面的代码生效，将参数添加到文件中过于靠后的位置会导致生效范围过小效果不佳
...
... 
# 通常CMakeList开头会进行一些类似变量定义的准备操作

 +message(WARNING "CXX compiler path: ${CMAKE_CXX_COMPILER}")
+add_compile_options(
+    -fprofile-generate=/data/storage/el2/base/files/pgoprofraw
...
+)
 ...
 # 编译选项需要写在添加构建目标，添加代码之类的操作前面才会对其生效
add_subdirectory("./path/to/code")
add_library(awesome_project code.cpp)
target_sources(awesome_project PRIVATE util.cpp)
...
```

其中 message 一行会在构建日志中打印 C++ 编译器的完整路径，方便您确定 LLVM 工具的位置在第四步中使用。`-fprofile-generate` 选项指定将收集到的数据写到盘上哪个文件夹。注意这里设置的 `/data/storage/el2/base/files/pgoprofraw` 是一个应用沙箱内路径，对应文件系统中的实际路径为 `/data/app/el2/100/base/[应用的bundle名， 如 com.example.application]/files/pgoprofraw`

后续步骤需要使用 lldb 连接到应用发送指令，需要在顶层 build-profile.json5 中临时指定 app.products.buildOption.debuggable 字段为 true 允许 debug。注意这里只是配置应用允许debug，跟打包时选择的 Release/Debug 模式不是同一个配置。可参考下方例子配置
...
以上两处添加完成后，尽可能贴近正式发布应用时的配置进行打包，如选择 Release 模式等。

2. 训练运行：如下图所示点击 Debug 按钮构建并启动应用

![alt text](./figures/性能调优-pgo启动.png)

点击下方 debug 面板中的 lldb，进入 lldb 工具命令行，点击左侧按钮禁用断点

![alt text](./figures/性能调优-lldb-cmd.png)

在 lldb 命令行中输入 `process handle`，获取所有可能导致程序暂停的信号量名称

![alt text](./figures/性能调优-信号.png)
...
![alt text](./figures/性能调优-nostop.png)
...
3. 整理运行数据：将 pgoprofraw 文件夹下载到电脑上
...
4. 二次反馈编译：使用采集到的profile信息指导编译器构建高性能应用。注意在打包前恢复步骤1中临时添加的配置。参考下面的例子修改您的编译选项

```cmake
add_compile_options(
    -fprofile-use=/path/to/pgo.profdata
)
```

其中 `/path/to/pgo.profdata` 是到步骤三生成的 `pgo.profdata` 文件的路径。在 Windows 上首先将 `pgo.profdata` 和代码放在同一个盘上，路径开头不需要写盘符，路径使用 / 分隔。
