...
2. 如果您使用的是 `RNApp` 启动的 **ReactNative** 框架，那么这一步可以跳过，`RNApp` 默认开启了 Metro。如果您使用的是 `RNSurface` 启动的 **ReactNative** 框架，您就需要在初始化创建 `rnInstance` 后执行创建一个 `RNComponentContext`，并在 `context` 的 `devToolsController` 中增加如下的事件监听，并删除原有的 `RNInstance` 实例，重新启动一个新的 `RNInstance` 实例，即可启用热加载的功能：
...
### 如何支持原生scroll嵌套RN scroll？
以 0.72.83 版本为例，需修改react-native-openharmony/src/main/cpp/RNOHCorePackage/ComponentInstances/ScrollViewComponentInstance.cpp文件。尽管不同版本间的具体修改点可能存在差异，但核心逻辑是一致的 —— 需在setNestedScroll函数中传入ARKUI_SCROLL_NESTED_MODE_PARENT_FIRST与ARKUI_SCROLL_NESTED_MODE_SELF_FIRST这两个参数。
