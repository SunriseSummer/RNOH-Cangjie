...
export class ImageLoaderTurboModule extends UITurboModule {
  ...
  public async getSize(uri: string): Promise<number[]> {
    ...
    console.error(`[getSize] ArkTS Test Start`);
    if (uri && uri.length > 0) {
      ...
      try {
        ...
        console.error(`[getSize] getSize: ${endTime - startTime} miliseconds`);
        return [imageInfo.size.width, imageInfo.size.height]
      } catch (e) {
        console.error(`[getSize] getImageInfo exception: ${e}`)
        throw 'Cannot get info of the image'
      }
    } else {
