...
napi_value initializeArkTSBridge(napi_env env, napi_callback_info info) {
  ArkJS arkJs(env);
  auto args = arkJs.getCallbackArgs(info, 1);
  auto bridgeHandlerRef = arkJs.createNapiRef(args[0]);
...
extern "C" void onImageSourceMapUpdate(unsigned long rnInstanceId, const char* remoteUri, const char* fileUri) {
  try {
    const std::string messageName = "UPDATE_IMAGE_SOURCE_MAP";
    folly::dynamic messagePayload = folly::dynamic::object();
    std::string strRemoteUri = remoteUri;
    std::string strFileUri = fileUri;
    messagePayload["remoteUri"] = strRemoteUri;
    messagePayload["fileUri"] = strFileUri;
